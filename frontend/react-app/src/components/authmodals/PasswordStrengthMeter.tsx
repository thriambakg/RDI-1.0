...
function scorePassword(password: string): { score: number; label: string } {
  if (!password || password.length === 0) return { score: 0, label: '' }
  let score = 0
  ...
  return (
    <div className="password-strength" role="status" aria-live="polite">
      <div className="password-strength__bars">
        {[1, 2, 3, 4].map((i) => (
          <div key={i} className={`password-strength__bar ${i <= score ? `password-strength__bar--${score}` : ''}`} />
        ))}
      </div>
      <span className={`password-strength__label password-strength__label--${score}`}>{label}</span>
    </div>
  )
}
