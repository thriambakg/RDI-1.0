...
The main workflow runs Terraform with **skip_proxy_build** and **skip_agent_build** set to true, so it does not build or upload proxy/agent binaries. For a **new environment** (first apply that creates proxy/Wavelength EC2), run the **Deploy EC2 Code** workflow once after the first successful infra apply so binaries are in S3 and instances can pull them (or run Terraform locally once without the skip flags to build and upload).
